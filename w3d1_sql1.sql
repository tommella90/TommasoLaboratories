...
use sakila;
SELECT * from film;
SELECT distinct name as "Language" FROM sakila.language title;
...
-- 10 In the order table, which account_ids were responsible for 
...
-- 12 In the trans table, show the trans_id, date, type and amount
...
SELECT count(client_id) as 'client N', district_id from bank.client
...
select type, count(card_id) as 'Amount' from bank.card
group by(type) 
...
-- 21 From the previous result, modify your query so that it returns only one row,
-- with a column for incoming amount, outgoing amount and the difference.
WITH group1 AS (
...
-- 22 Continuing with the previous example, rank the top 10 account_ids based on their difference.
with gr1 as (
